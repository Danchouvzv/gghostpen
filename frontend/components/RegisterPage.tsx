...
import { Ghost, UserPlus, Mail, Lock, User, ArrowRight, LogIn } from 'lucide-react';
...
const RegisterPage: React.FC<RegisterPageProps> = ({ onRegister, onSwitchToLogin, isLoading = false }) => {
  const { t } = useLanguage();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
      setError(t.auth.registerError); // Можно добавить отдельный ключ для этой ошибки
      return;
    }
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="bg-white border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-8">
          ...
          <div className="mt-6 pt-6 border-t-2 border-black">
            ...
            >
              <LogIn size={16} />
              <span>{t.auth.switchToLogin.split('? ')[1]}</span>
            </button>
          </div>
        </div>
      </div>
